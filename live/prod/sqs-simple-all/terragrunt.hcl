...
inputs = merge(
  local.prod_config.inputs,
  {
    queue_details = [
      {
        queue_name                 = "${local.prod_config.inputs.stage}_${local.prod_config.inputs.prefix}_sample",
    ...
    tags = {
      Name = "${local.prod_config.inputs.stage}-${local.prod_config.inputs.method}-${local.prod_config.inputs.prefix}-sqs-simple"
    }
  }
)
