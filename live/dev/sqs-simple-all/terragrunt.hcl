...
inputs = merge(
  local.dev_config.inputs,
  {
    queue_details = [
        {
            queue_name = "${local.dev_config.inputs.stage}_${local.dev_config.inputs.prefix}_sample", 
    ...
    tags = {
        Name = "${local.dev_config.inputs.stage}-${local.dev_config.inputs.method}-${local.dev_config.inputs.prefix}-sqs-simple"
    }    
  }
)